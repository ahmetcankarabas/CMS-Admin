...
if(isset($_POST['form1'])) {
	...
	if($valid == 1) {
		...
		if($path == '') {
			$statement = $pdo->prepare("UPDATE tbl_slider SET heading=?, subheading=?, content=?, button_text=?, button_url=?, position=?, status=? WHERE id=?");
    		$statement->execute(array($_POST['heading'],$_POST['subheading'],$_POST['content'],$_POST['button_text'],$_POST['button_url'],$_POST['position'],$_POST['status'],$_REQUEST['id']));
		} else {
        	...
        	$statement = $pdo->prepare("UPDATE tbl_slider SET photo=?, heading=?, subheading=?, content=?, button_text=?, button_url=?, position=?, status=? WHERE id=?");
    		$statement->execute(array($final_name,$_POST['heading'],$_POST['subheading'],$_POST['content'],$_POST['button_text'],$_POST['button_url'],$_POST['position'],$_POST['status'],$_REQUEST['id']));
		}	   
...
<section class="content-header">
	...
	<div class="content-header-right">
		<a href="slider.php" class="btn btn-primary btn-sm">Hepsini Göster</a>
	</div>
</section>
				...
				<div class="box box-info">
					<div class="box-body">
						...
						<div class="form-group">
							...
							<div class="col-sm-6">
								<input type="text" autocomplete="off" class="form-control" name="subheading" value="<?php echo $subheading; ?>">
							</div>
						</div>
						<div class="form-group">
							<label for="" class="col-sm-2 control-label">İçerik </label>
							<div class="col-sm-6">
						...
						<div class="form-group">
							<label for="" class="col-sm-2 control-label">Pozisyon (Rastgele Seçebilirsiniz) </label>
							<div class="col-sm-6">
