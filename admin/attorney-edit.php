...
if(isset($_POST['form1'])) {
	...
	if($valid == 1) {
		...
		$statement = $pdo->prepare("SELECT * FROM tbl_attorney WHERE slug=? AND name!=?");
		$statement->execute(array($slug,$_POST['current_attorney_name']));
		...
		if($path == '' && $path1 == '') {
			$statement = $pdo->prepare("UPDATE tbl_attorney SET name=?, slug=?, designation_id=?, degree=?, detail=?, facebook=?, twitter=?, linkedin=?, youtube=?, google_plus=?, instagram=?, flickr=?, address=?, practice_location=?, phone=?, email=?, website=?, status=?, meta_title=?, meta_keyword=?, meta_description=? WHERE id=?");
		...
		if($path != '' && $path1 == '') {
			...
			$final_name = 'attorney-'.$_REQUEST['id'].'.'.$ext;
        	move_uploaded_file( $path_tmp, '../assets/uploads/'.$final_name );

			$statement = $pdo->prepare("UPDATE tbl_attorney SET name=?, slug=?, designation_id=?, photo=?, degree=?, detail=?, facebook=?, twitter=?, linkedin=?, youtube=?, google_plus=?, instagram=?, flickr=?, address=?, practice_location=?, phone=?, email=?, website=?, status=?, meta_title=?, meta_keyword=?, meta_description=? WHERE id=?");
		...
		if($path == '' && $path1 != '') {
			...
			$final_name1 = 'attorney-banner-'.$_REQUEST['id'].'.'.$ext1;
        	move_uploaded_file( $path_tmp1, '../assets/uploads/'.$final_name1 );

			$statement = $pdo->prepare("UPDATE tbl_attorney SET name=?, slug=?, designation_id=?, banner=?, degree=?, detail=?, facebook=?, twitter=?, linkedin=?, youtube=?, google_plus=?, instagram=?, flickr=?, address=?, practice_location=?, phone=?, email=?, website=?, status=?, meta_title=?, meta_keyword=?, meta_description=? WHERE id=?");
		...
		if($path != '' && $path1 != '') {
			...
			$statement = $pdo->prepare("UPDATE tbl_attorney SET name=?, slug=?, designation_id=?, photo=?, banner=?, degree=?, detail=?, facebook=?, twitter=?, linkedin=?, youtube=?, google_plus=?, instagram=?, flickr=?, address=?, practice_location=?, phone=?, email=?, website=?, status=?, meta_title=?, meta_keyword=?, meta_description=? WHERE id=?");
    		$statement->execute(array($_POST['name'],$slug,$_POST['designation_id'],$final_name,$final_name1,$_POST['degree'],$_POST['detail'],$_POST['facebook'],$_POST['twitter'],$_POST['linkedin'],$_POST['youtube'],$_POST['google_plus'],$_POST['instagram'],$_POST['flickr'],$_POST['address'],$_POST['practice_location'],$_POST['phone'],$_POST['email'],$_POST['website'],$_POST['status'],$_POST['meta_title'],$_POST['meta_keyword'],$_POST['meta_description'],$_REQUEST['id']));
		}

	    $success_message = 'Attorney is updated successfully!';
	}
}
?>
...
if(!isset($_REQUEST['id'])) {
	...
	$statement = $pdo->prepare("SELECT * FROM tbl_attorney WHERE id=?");
...
<section class="content-header">
	...
	<div class="content-header-right">
		<a href="attorney.php" class="btn btn-primary btn-sm">Hepsini Göster</a>
	</div>
</section>
	...
	<div class="row">
		<div class="col-md-12">
			...
			<form class="form-horizontal" action="" method="post" enctype="multipart/form-data">
				...
				<input type="hidden" name="current_attorney_name" value="<?php echo $name; ?>">
				<div class="box box-info">
					<div class="box-body">
						...
						<div class="form-group">
				            <label for="" class="col-sm-2 control-label">Sabit Kategori <span>*</span></label>
				            <div class="col-sm-3">
						...
						<div class="form-group">
							...
							<div class="col-sm-9" style="padding-top:5px">
								<img src="<?php echo BASE_URL; ?>assets/uploads/<?php echo $photo; ?>" alt="Attorney Photo" style="width:200px;">
							</div>
						</div>
						<div class="form-group">
							<label for="" class="col-sm-2 control-label">Fotoğaf Seç <span>*</span></label>
							<div class="col-sm-9" style="padding-top:5px">
						...
						<div class="form-group">
							...
							<div class="col-sm-9" style="padding-top:5px">
								<img src="<?php echo BASE_URL; ?>assets/uploads/<?php echo $banner; ?>" alt="Attorney Banner" style="width:200px;">
							</div>
						</div>
						<div class="form-group">
							<label for="" class="col-sm-2 control-label">Banner <span>*</span></label>
							<div class="col-sm-9" style="padding-top:5px">
								<input type="file" name="banner">(Sadece jpg, jpeg, gif ve png )
							</div>
						</div>
						<div class="form-group">
							<label for="" class="col-sm-2 control-label">Sıra No: (Boş bırakılabilir) </label>
							<div class="col-sm-6">
								<input type="text" autocomplete="off" class="form-control" name="degree" value="<?php echo $degree; ?>">
							</div>
						</div>
						<div class="form-group">
							<label for="" class="col-sm-2 control-label">Açıklama </label>
							<div class="col-sm-9">
						...
						<div class="form-group">
							<label for="" class="col-sm-2 control-label">Addres </label>
							<div class="col-sm-6">
								<textarea class="form-control" name="address" style="height: 140px"><?php echo $address; ?></textarea>
							</div>
						</div>
						<div class="form-group">
							<label for="" class="col-sm-2 control-label">Lokasyon</label>
							<div class="col-sm-6">
						...
						<div class="form-group">
							<label for="" class="col-sm-2 control-label">Email Addres </label>
							<div class="col-sm-6">
				        ...
				        <div class="form-group">
				            <label for="" class="col-sm-2 control-label">Aktif/Pasif </label>
				            <div class="col-sm-6">
